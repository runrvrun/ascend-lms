...
import { createGrowthPlan, updateGrowthPlan, deleteGrowthPlan, toggleGrowthPlanComplete } from "../../growth-plan/actions"
...
function GrowthPlanRow({ item }: { item: GrowthPlanItem }) {
  ...
  return (
    <div className={`flex items-start gap-3 rounded-xl border p-4 transition-colors ${isDone ? "border-green-100 bg-green-50" : "border-slate-100 bg-white"}`}>
      <button onClick={handleToggle} disabled={pending} className="mt-0.5 shrink-0 disabled:opacity-50" title={isDone ? "Mark incomplete" : "Mark complete"}>
        {isDone
      ...
      <div className="min-w-0 flex-1">
        {editing ? (
          ...
          <>
            <p className={`text-sm font-medium ${isDone ? "line-through text-slate-400" : "text-slate-800"}`}>{item.title}</p>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {item.pathwayName && (
                <span className="rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600">{item.pathwayName}</span>
              )}
              {isConfirmed && (
...
function GrowthPlanAddForm({ pathwayId }: { pathwayId?: string }) {
...
function GrowthPlanPanel({
  ...
  for (const item of allItems) {
    ...
    if (!groups.has(key)) groups.set(key, { label: item.pathwayName, items: [] })
  ...
  const pathwayGroups = [...groups.entries()]
    .filter(([k]) => k !== null)
    .sort((a, b) => {
      // Current pathway first
      if (a[0] === currentPathwayId) return -1
      if (b[0] === currentPathwayId) return 1
    ...
    <div className="h-full overflow-y-auto p-6 md:p-8">
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Growth Plan</h2>
            <p className="text-sm text-slate-500">
              After completing <span className="font-medium text-slate-700">{currentPathwayName}</span>, my plans are:
            </p>
          </div>
        </div>

        <div className="flex flex-col gap-8">
          {/* Current pathway section first */}
          {pathwayGroups
            .filter(([k]) => k === currentPathwayId)
            .map(([pathwayId, group]) => (
              <section key={pathwayId}>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <GrowthPlanAddForm pathwayId={currentPathwayId} />
                </div>
              </section>
            ))}
          {/* If no items for current pathway yet, show add form */}
          {!groups.has(currentPathwayId) && (
            <section>
              <div className="mb-3 flex items-center gap-2">
                <Target size={15} className="text-blue-400" />
                <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">{currentPathwayName}</h3>
              </div>
              <div className="flex flex-col gap-2">
                <GrowthPlanAddForm pathwayId={currentPathwayId} />
              </div>
    ...
    <div className="flex h-screen flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden w-72 shrink-0 flex-col overflow-y-auto border-r border-slate-200 bg-white md:flex">
          ...
          {isPathwayComplete && (
            <div className="border-t border-slate-100">
              <button
                onClick={() => setSelected({ kind: "growth-plan" })}
                className={`flex w-full items-center gap-2 px-4 py-3 text-left text-sm font-medium transition-colors ${
                  selectedId === "growth-plan"
                    ? "bg-blue-50 text-blue-700"
                    : "text-slate-600 hover:bg-slate-50"
                }`}
              >
                <Target size={15} className={selectedId === "growth-plan" ? "text-blue-500" : "text-slate-400"} />
                Growth Plan
                {allGrowthPlans.filter((g) => !g.completedAt).length > 0 && (
                  <span className="ml-auto text-xs text-slate-400">
                    {allGrowthPlans.filter((g) => !g.completedAt).length} pending
                  </span>
                )}
              </button>
            </div>
        ...
        <main className="flex-1 overflow-hidden bg-white">
          {selected?.kind === "growth-plan" ? (
            <GrowthPlanPanel
              allItems={allGrowthPlans}
              currentPathwayId={pathway.id}
              currentPathwayName={pathway.name}
            />
          ) : (
            <ContentViewer
              selection={selected}
