...
import { Plus, Pencil, Trash2, X, BookOpen, ArrowRight, Search, ChevronDown, Copy, Eye, BarChart3, Download, Upload } from "lucide-react"
import { createCourse, updateCourse, deleteCourse, toggleCourseStatus, setCourseTrainers, duplicateCourse, importCourse, CourseFormData } from "./actions"
...
function ImportCourseModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">Import Course</h2>
          <button onClick={onClose} className="rounded-lg p-1 hover:bg-slate-100"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <p className="text-sm text-slate-500">
            Select a course export JSON file (from the Export action on another Ascend LMS instance). This creates a new,
            draft course with its contents, tests, and assignment — nothing is added to any pathway or trainer assignment automatically.
          </p>
          <input
            required
            type="file"
            ...
            className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm file:font-medium"
          />
          {error && <p className="text-xs text-red-500">{error}</p>}
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
            <button type="submit" disabled={pending || !file} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">
              ...
              <tr key={c.id} className="group hover:bg-slate-50">
                  ...
                  <td className="sticky right-0 bg-white px-4 py-3 shadow-[-8px_0_12px_-6px_rgba(0,0,0,0.06)] group-hover:bg-slate-50">
                    <ActionsMenu items={[
                      ...
                      { label: "Report", icon: <BarChart3 size={14} />, href: `/admin/course/${c.id}/report` },
                      ...
                      { label: "Export", icon: <Download size={14} />, href: `/api/admin/course/${c.id}/export` },
